...
int main(int argc, char** argv) {
    ...
    config.n_rx_ant = 1;
    config.n_layers = 1;
    config.max_blocks_per_sinr = 1000;
    ...
    std::cout << "Channel: TDL-A, DS=30ns, perfect CSI\n";
    std::cout << "MCS " << config.mcs_index << ": " << mod_name
              << ", Qm=" << qm_int << ", R=" << R << " (" << (int)(R*1024) << "/1024)\n";
    std::cout << "PRBs=" << config.n_rb << ", TBS=" << tbs << " bits, SISO\n";
    ...
    std::string out_file = "bler_mcs27_tdla_perfect_csi.csv";
