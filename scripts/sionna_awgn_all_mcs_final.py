...
def run_mcs_bler(mcs):
    ...
    if eff_rate < 1.0/5.0:
        print(f"  Skipping (R={eff_rate:.3f} < 0.2: Sionna does not support repetition coding)", flush=True)
        return []
...
def main():
    ...
    out_csv = os.path.join(out_dir, 'sionna_all_mcs_awgn_final.csv')
    with open(out_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['mcs','qm','rate','tbs','snr_db','bler','n_blocks','n_errors'])

    print(f"Sionna AWGN BLER - All MCS, nPRB={n_prb}, SISO, ldpc_iters={num_ldpc_iters}", flush=True)
    print()
