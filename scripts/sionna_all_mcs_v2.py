...
def run_mcs_bler(mcs):
    ...
    print(f"  Qm={qm}, R_table={R:.3f}, TBS={tbs}, n_coded_bits={n_coded_bits}, eff_rate={eff_rate:.3f}", flush=True)

    if eff_rate < 1.0/5.0:
        print(f"  Skipping: effective rate {eff_rate:.3f} < 0.2 (Sionna LDPC5G does not support repetition coding)", flush=True)
        return []

    try:
        tb_enc = TBEncoder(
            target_tb_size=tbs,
            num_coded_bits=n_coded_bits,
            target_coderate=R,
...
def main():
    ...
    out_csv = os.path.join(out_dir, 'sionna_all_mcs_v2_awgn.csv')
    with open(out_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['mcs','qm','rate','tbs','snr_db','bler','n_blocks','n_errors'])

    print(f"Sionna AWGN BLER - All MCS (0-27), nPRB={n_prb}, SISO", flush=True)
